...
4.如果实在实在是没有弄出来，请联系我，可以共同交流，我的邮箱：xiaozuoadamszju@163.com
...
u16 CRC16(u8 *puchMsgg,u8 usDataLen);//用于计算CRC校验
 ...
 */
 ...
 函数输入：两个指针，pointer_1指向用来存放输入信息帧的数组，pointer_2用			来指向存放输出信息帧的数组（两个数组的第一个元素都用来存放			信息帧的有效字节个数）后面的元素按照Modbus协议组织	
 函数输出：无											
 */
 ...
 */
 ...
 /* 
	函数功能：读取输入状态
 	函数输入：两个指针，pointer_1指向用来存放输入信息帧的数组， pointer_2用来指向存放输出信息帧的数组（两个数组的第一个元素都用来			  存放信息帧的有效字节个数）后面的元素按照Modbus协议组织	
 	函数输出：无		
 */					
 ...
 /* 
	函数功能：读取保持寄存器
 	函数输入：两个指针，pointer_1指向用来存放输入信息帧的数组，pointer_2用来指向存放输出信息帧的数组
 				  （两个数组的第一个元素都用来存放信息帧的有效字节个数）后面的元素按照Modbus协议组织
 	函数输出：无
 */
...
/* 
 ...
 /* 
	...
	函数输出：无。                                                                         
 */
tMulReg(u8 *pointer_1,u8 *pointer_2)//pointer_1用作输入，pointer_2用作输出
 {
...
函数输入：第一个参数Mode用来指示哪一类错误， pointer用来指向存放输出信息帧的数组（两个数组的第一个元素都用来存放信息帧的有效字节个数）
		  	 后面的元素按照Modbus协议组织。														    
函数输出：无。 
*/
